...
confirmed one, which is precisely the confusion BR148 exists to prevent:
...
## What may be proposed, and what may not (BR148, #77)
...
BR148 is deliberately narrower than decision 11's family reads, because a
...
| 2_business | **BR148 added.** Resolves [#77](./open-questions.md), which scope 62's audit raised |
...
- **Migration:** `supabase/migrations/0052_player_record_self_correction.sql` — `player_record_correction`, a trigger enforcing BR148's adult-and-own-registration gate, RLS (own read via `app_my_person_ids()`, officer read via the five confirming roles, propose via an RLS insert policy, no update policy at all), and `app_review_player_record_correction()`.
...
| -------- | ------------------------------------------ |
| Self-service correction for an adult player, six fields | A guardian proposing on a minor's behalf — BR148 is deliberately narrower than decision 11's family reads |
