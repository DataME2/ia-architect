...
| ~~T2.2~~ | **A club holds at least two administrators.** No constraint enforced the floor; a club with one could not remove that one and could not get back in if they left. **Done** — [scope 55](../scope/55_a_club_keeps_two_administrators.md), which also found that 0002's manage policy let an admin delete a membership row directly, so the *last*-administrator guard had been bypassable since 0015 | BR124 | S | The second-last administrator cannot be removed |
