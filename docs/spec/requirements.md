...
| NFR-16 | **Performance budgets** — queue render, pack generation at ~700 registrations | [Stated](../annexes/performance-budgets.md) | **Partly measured** | BR5's duplicate detection was quadratic on three screens — **495 ms → 4.8 ms at 1,400 people**, and gated by a test that asserts the *curve* rather than a millisecond ceiling. The People directory ([scope 59](../scope/59_a_directory_that_does_not_read_the_whole_club.md)) no longer fetches the whole club to show one page of it, but that is a code-review argument, not a measurement. Nothing is measured against a database, and end-to-end pack generation still is not |
| NFR-17 | **Availability and backup/restore** | [RPO 24h, RTO 4h](../annexes/backup-and-restore.md) | **Partly verified** | Targets stated, and a restore **rehearsed on every change** — dump, restore into an empty database, count what came back with the RLS policies checked first, because a restore that keeps the tables and loses them reads as a clean one. Proves nothing about Supabase's own backups, the real restore time, whether accounts survive, or the Storage bucket — all of which need the production project ([scope 49](../scope/49_an_environment_of_its_own.md)) |
