...
| **Public Event View** | The account-free, published schedule/draw/results of a Carnival/Grassroots Event, club/team-level only by default (BR26) — the scoped public exception to tenant isolation (Principle P6, BR27) |
...
| BR26 | A Carnival/Grassroots Event's Public Event View shows club/team-level schedule, draws, and results only — individual player names are never published unless the Events Coordinator explicitly marks the event as adult/open-age and opts in per event | Carnival & event management | Adopted interpretation ([open question #23](../../scope/open-questions.md)); protects minors' identity by default in unauthenticated, public content, in the spirit of Principle P4 |
