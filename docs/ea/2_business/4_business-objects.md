...
| **Committee Resolution** | A dated decision the Committee recorded, naming what was decided, who moved it (where the club names one), and which Committee Term it belongs to (BR123). **Append-only** — a corrected decision is a new resolution, never an edit, the same absence of an update path that makes a Payment one. `category` lets a caller like Voucher Program Enablement find the resolution that approved it without parsing free text; every other decision (buying equipment, hiring a coach) is recorded as `general` |
...
| **Payment Plan** | An agreement to pay a registration's total over several dated Installments, at a weekly, fortnightly or monthly cadence. Carries the total, who agreed it, and — when it ends early — the date it was cancelled, never a deletion (BR75). At most one is live per registration |
| **Installment** | One dated amount within a Payment Plan. The instalments of a plan sum to **exactly** its total (BR74), and the last one falls on or before the end of the season (BR76) |
| **Payment** | Money the club records having received against a registration: amount, date, method, and whatever reference ties it to reality outside the platform. **Append-only** — a refund or a correction is a new negative entry naming the one it reverses (BR77). Allocated to instalments oldest-first at read time, and never stored as an allocation |
...
| **Club Voucher Program Enablement** | A Club's decision to accept a Voucher Program for its own invoices — names the Committee Resolution that approved it (BR21, [scope 57](../../scope/57_the_committee_records_its_own_decisions.md)), and **enforced by the database itself**: a resolution cited must be recorded with category `voucher_program` and belong to the same club, and `registration_voucher` refuses any program the club has not enabled this way, matched trimmed and case-insensitive. **The pilot club's Committee has approved Queensland's Play On! program** (confirmed July 2026); what artifact evidences that approval is still open ([question #28](../../scope/open-questions.md)) |
| **Voucher** | A discount instrument redeemed against an invoice under a Club's enabled Voucher Program (e.g. a Queensland Play On! voucher), tracked by code, beneficiary, value, issue/use dates, status, applied invoice, and duplicate risk. Attached to a registration with the voucher **document** where the family has one, and moving `ATTACHED → VERIFIED \| REJECTED → CLAIMED`. **`ATTACHED` reduces nothing** (BR81): only verification applies the value, and it does so as a Payment rather than a discount field, so relief and receipts share one ledger. One code per club — a government voucher is single-use, and the same code on two children is a mistake or a duplicate claim |
...
| **Match Confirmation** | A Guardian's after-the-fact confirmation that a fixture their under-13 match official was appointed to actually happened and their child was there. Marks the fixture played; an optional score fills `fixture.goals_for`/`goals_against` only where the club has not already entered one. Never a substitute for BR119's payment verification (BR151) |
| **Claim Settlement Choice** | The official's family's choice, once a Referee Payment Claim is approved, between being paid (the club acts outside the platform, BR118) and having it credited toward the official's own next registration fee. Recorded against the claim; applying a chosen credit to a future season is a registrar or treasurer's own manual act, not automated (BR152) |
