...
- **Registration proves itself in three stages, and only stage 1 is
  ...
  | **2 — SQUADI synchronisation** | The same registration flowing through to SQUADI, removing the duplicate entry a parent or guardian performs manually today | **Approved system partner status with Football Queensland / Football Australia** — not a club-level API request, which was made and declined on 31 July 2026 (see the Football Queensland response resource, and [open questions #39](../../scope/open-questions.md), [#41](../../scope/open-questions.md)) |
