...
| **`clearance_lapse_vacancy`** | Clearance Lapse Vacancy | BR50. An assignment a lapsed Working with Children Check emptied, described **in words captured at the time**, and whether the holder and the coordinator have each been told. Unique per assignment, which is what makes "nobody is notified twice" a property of the schema rather than of whoever calls the sweep. Written only by `app_withdraw_lapsed_clearances`; no insert or update policy exists, so a club officer can neither fabricate a vacancy nor silence one |
| **`match_official_appointment`** | Match Official Appointment | Carries the **appointing party** (BR114), which is what BR16 pays on, and the reason recorded with any decline or withdrawal — without which nothing is recorded at all (BR42, BR112). `responded_by_person_id` carries **whose answer it is**: required of an under-18 official, where it must name a Parent/Guardian holding authority (BR113), and optional for an adult, who answers for themselves |
