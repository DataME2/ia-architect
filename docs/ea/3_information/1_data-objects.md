...
| **`registration_document`** | Person Document | Required documents attached to a registration; the file itself in Storage |
...
| **`submission_pack`** | Registration Submission Pack | Immutable and versioned (BR58): `version`, `generated_at`, `generated_by`, `club_id`, `season_id`, `channel`, `handed_over_at`. Rows are never updated after generation |
