...
| **Account identification** | C1, C19 | An administrator records which `Person` an account belongs to — one per account per club and one per account per Person, in both directions (BR106), asserted and never inferred from a matching email address (BR107, [decision 10](../../decisions/10_identity_is_asserted_never_inferred.md)). `app_club_accounts()` returns the name, `app_who_am_i()` answers for the caller, and both return null where there is no link | **No screen reads it.** `/registrar/access` still lists email addresses, no session strip shows a name, and the only way to make a link is to call the function. The database half is delivered and proved by 19 scenarios; the rendering half of [scope 29 WP1](../../scope/29_actors-access-and-permissions.md) is not started |
