...
| **Multitenant platform operations** | C10 | Tenant isolation (above), role-based access through `club_membership`, and per-season configuration | **Account creation and provisioning.** An admin can now grant and revoke roles at their own club (`/registrar/access`), but cannot create the account itself — that needs the Auth admin API. Tenant provisioning is still four hand-typed SQL statements ([annex](../../annexes/tenant-provisioning.md)); the owner-issued onboarding link of [decision 7](../../decisions/7_tenant-provisioning-by-owner-issued-invitation.md) is unbuilt; **central super-administration is unbuilt and needs its own decision record first** — it is a P5 exception. No club branding | **Partial** |
