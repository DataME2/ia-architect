...
| **Multitenant platform operations** | C10 | Tenant isolation (above), role-based access through `club_membership`, and per-season configuration | **Account creation.** Roles can be granted and revoked at a club (`/registrar/access`) and clubs can be provisioned (`/platform`), but neither can create an `auth.users` row — that needs the Auth admin API, so a person always signs up themselves first. The owner-issued onboarding link of [decision 7](../../decisions/7_tenant-provisioning-by-owner-issued-invitation.md) is still unbuilt, and there is no club branding | **Partial** |
