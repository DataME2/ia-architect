...
| **Club governance & administration** | C19 | The committee as a record: who holds which office, elected at which AGM, serving until the next. An overdue AGM is flagged rather than hidden (BR86), holders must be adults (BR87), and a missing card is shown rather than refused (BR88). Now also what it has decided (BR123) — a dated, append-only resolution — and its Voucher Program approvals (BR21), enforced by a database trigger on `registration_voucher` rather than a check on this screen (scope 57) | **Delivered** |
