...
| `0042` | The administrator invitation requires a named individual (scope 48, WP3): rewrites `grant_club_role` to refuse `admin` to an account not yet linked to a Person (`account_person`) — the schema's own definition of "a named individual", since nothing in an email address distinguishes a shared mailbox from a personal one. Skipped for an account that already holds admin, so this gates the grant, not continued use |
| `0041` | The six-monthly WWCC reminder (scope 48, WP2): `clearance.reminder_sent_at`, a due-list report and a plain RLS-backed write to record one sent. Finds that BR97's read-only state gates nothing at the RLS layer yet, so there was no exception to carve for it |
...
| `0046` | The vacancy a lapse leaves: BR50's last clause — `clearance_lapse_vacancy` records what a lapse emptied, unique per assignment so nobody is notified twice, with holder and coordinator marked apart. Also stops two older guards refusing a *removal*: 0025's conflict check and 0010's team-member card check both blocked the very withdrawal BR50 exists to make |
| `0047` | A superseded fee schedule is history: its rates cannot be added to, changed or removed. The one in force stays editable — a rate mistyped this morning must be correctable — and `app_fee_schedule_on` decides which is which, rather than the trigger re-deriving it |
| `0048` | A club keeps two administrators (BR124): the floor moves from `revoke_club_role` onto `club_membership`, because 0002's manage policy let an admin delete the row directly — so the *last*-administrator guard had been bypassable since 0015. Covers demotion as well as deletion, and stands aside for a club's own cascade |
| `0049` | The Committee records its own decisions (BR123): `committee_resolution`, append-only, written by `committee` or `admin` — the first thing that role could write. Found BR21 unenforced since 0008 while building it: `club_voucher_program_enablement` gives it a gate, citing a resolution that must be the same club's and recorded with category `voucher_program`, checked by trigger rather than a composite key since neither `committee_term` nor `committee_resolution` carries a `(club_id, id)` unique key. `registration_voucher` now refuses any program the club has not enabled this way |
