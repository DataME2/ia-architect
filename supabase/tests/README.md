...
| `50` | Committee resolutions and Voucher Programs — `committee` and `admin` may record a resolution and no lesser role may, a resolution is **append-only** and cannot be filed against another club's Committee Term, enabling a Voucher Program requires citing a resolution of its own category (`voucher_program`) from its own club, and the BR21 gate on `registration_voucher` accepts an enabled program matched trimmed and case-insensitively while refusing every other |
